# Реализован обмен между Tomcat сервером и клиентом. На сервере используется сервлет NewServlet.
На сервер отправляется get запрос с параметрами кредита. В сервлете производится прием данных,
вычисление кредита, и возврат результатов в виде json строки. После получения клиетом производится 
